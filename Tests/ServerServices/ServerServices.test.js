...
describe('Server Services', () => {
  ...
  it("Tasks GetById - Should return the task with the same id", async () => {
    let data = await Services.tasks.getById({ id: "11", data: MockedValues.tasks })

    expect(data).toMatchObject({
      status: 200,
      json: MockedValues.tasks[10]
    })
  })

  it("Users Get All - Should return all users", async () => {
    let { json: { data } } = Services.users.get({ data: MockedValues.users })
  ...
  it("Users GetById - Should return the user with the id", async () => {
    ...
    expect(data).toMatchObject({
      status: 200,
      json: MockedValues.users[4]
    })
  })

  it("Users GetTasksFromUser - Should return the user's tasks", async () => {
    let { json: { data } } = Services.users.getTasksFromUser({ id: 3, data: MockedValues.tasks })
    expect(data.length).toBe(2)
  })

  it("Users GetTasksFromUser - Having complete params should return the tasks that are completed", async () => {
    let { json: { data } } = Services.users.getTasksFromUser({ id: 1, completed: true, data: MockedValues.tasks })
    expect(data.length).toBe(5)
  })

  it("Users GetTasksFromUser - Having title params should return the tasks that contains that string", async () => {
    let { json: { data } } = Services.users.getTasksFromUser({ id: 1, title: "molestiae perspiciatis ipsa", data: MockedValues.tasks })
